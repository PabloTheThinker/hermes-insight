...
2. insight_recall(query, observations?) — refuse when usable is false
3. insight_perceive(situation, observations?)
4. For consequential/multi-step work: insight_plan(situation, observations?)
5. If usable: open task → typed events/work → close with used_pattern_ids
6. After ≥3 comparable tasks: insight_learn(materialize=false), inspect counterexamples
7. If novel/weak: deep=true or insight_cycle
8. Promote only verified-local procedure through reviewed skill_manage
9. insight_remember for one durable claim (never chat dumps)
```
